...
{#if selectedId && selectedId === belongingId}
	<div
		data-cy-id={TEST_IDS.YesNoPopupId}
		id={belongingId}
		class="absolute cursor-default border border-solid border-amadeusgray rounded items-center bg-white top-9 right-2 z-10 w-max shadow-lg"
		use:clickOutside
		on:click_outside={(e) => sendCommand(e, Commands.ActionCanceled)}
	>
		<!-- svelte-ignore a11y-click-events-have-key-events -->
		<div
			data-cy-id={TEST_IDS.YesNoPopupMainDivId}
			on:click={(e) => sendCommand(e, Commands.StopAction)}
